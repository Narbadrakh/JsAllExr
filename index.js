...
// ----- 7. tokenize -----
// split() функц нь string " " тэмдэгтээр (delimiter) зааглаж массив буву array үүсгэдэг.
function tokenize(expr) {
   return expr.split(" ");
}

console.log("7. ", tokenize("3 + 4"));
...
// ----- 8. calcOne -----
// tokenize() функцээр string-ийг token-ууд болгон хувааж,
// applyOp() функцээр token-уудыг тооцоолно.
...
// tokenize() функцээр string-ийг token-ууд болгон хувааж,
// applyOp() функцээр token-уудыг тооцоолно.
// left-to-right буюу зүүнээс баруун тийш дараалалтайгаар тооцоолно.

function calculate(expr) {  // expr нь "8 - 2 + 1" гэх мэт string хэлбэрийн утгыг нэрлэж буй нь
   let tokens = tokenize(expr); // tokenize() функцыг ашиглан string-ийг token-ууд болгож буй нь
   let result = Number(tokens[0]); // tokens массивын эхний элементийг авч result-д утга оноож буй нь

   // i += 2 гэдэг нь i = i + 2 гэсэнтэй адил бичиглэл
   for (let i = 1; i < tokens.length; i += 2) {

      // tokens массивын i+1 индексийн элементийг тоон утгад хөрвүүлж nextNumber-д оноож буй ньƒ
      let nextNumber = Number(tokens[i + 1]);

      // tokens массивын i индексийн элементийг op-д оноож буй нь
      let op = tokens[i];

      // applyOp() функцээр result, op, nextNumber-ийг тооцоолон result-д хадгалж буй нь
...
// Math.round() функц нь тоог хамгийн ойрын бүхэл тоогоор тоймлодог. 100-аар үржүүлж, Math.round() функцээр бүхэл тоонд
// тоймолж, 100-д хувааж буцааж буй нь 2 оронтойгоор тоймлон харуулахад ашиглагдаж байна.
function round2(n) {
   return Math.round(n * 100) / 100; 
}

console.log("10. ", round2(5.005));
...
return db.map(counntry => counntry.name);
}
...
return db.map(counntry => counntry.capital);
}
console.log(allCapitals(countries)[0]);
console.log(allCapitals(countries).length);
console.log(allCapitals([countries[25]]));
// console.log(allCapitals(countries)[0]);
...
const total = db.reduce((sum, country)=> country.population + sum,0);
...
function averageArea(db){
  const total = db.reduce((sum, country) => sum + country.area, 0);
...
return [... db].sort((a, b) => b.population - a.population)
...
if (density > bestDensity) {return country;}
else { return best;}
});
return winner.name;
}
